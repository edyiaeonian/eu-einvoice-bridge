...
官方 UNTDID 5305 允許的代碼為 `AE L M E S Z G O K B`。本專案支援其中七個,排除 `L`、`M`(西班牙加那利群島、休達與梅利利亞)與 `B`(義大利),因與波蘭情境無關。

**免稅理由的約束為雙向,已對照 vendored Schematron 查證:**

| 稅別 | 免稅理由 | 依據 |
|---|---|---|
| `E` 免稅、`AE` 逆向課稅、`G` 出口至歐盟外、`O` 不課稅、`K` 歐盟境內供應 | **必須有**(代碼或文字擇一) | BR-E-10、BR-AE-10、BR-G-10、BR-O-10、BR-IC-10 |
| `S` 標準稅率、`Z` 零稅率 | **不得有** | BR-S-10、BR-Z-10 |

注意 `K`(歐盟境內供應)的規則前綴是 **BR-IC-**,與代碼字母不同。
...
- **稅別不支援 `L`、`M`(西班牙)與 `B`(義大利)**,無法表達該三地的合法發票
- 無使用者介面
